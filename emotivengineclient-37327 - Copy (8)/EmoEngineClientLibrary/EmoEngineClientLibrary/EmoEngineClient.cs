...
namespace EmoEngineClientLibrary
{
    ...
    public class EmoEngineClient : INotifyPropertyChanged, IDisposable
    {
        ...
        public void Coba()
        {
            typeof(Button).GetMethod("set_IsPressed", BindingFlags.Instance | BindingFlags.NonPublic).Invoke(this.btnB, new object[] { true });
        }
        ...
        void dataPollingWorker_DoWork( object sender, DoWorkEventArgs e )
        {
            ...
            while( !dataPollingworker.CancellationPending )
            {
                ...
                if( currentData != null )
                {
                    ...
                    int _bufferSize = currentData[EdkDll.EE_DataChannel_t.TIMESTAMP].Length;
                    count += _bufferSize;
                    if(count>128)
                    {
                        count = _bufferSize;
                    ...
                    //Console.WriteLine("buffersize: " + _bufferSize);
                    for (int i = 0; i < _bufferSize; i++)
                    {
                        double[] input = new double[14];
                        input[0] = currentData[EdkDll.EE_DataChannel_t.AF3][i];
                        input[1] = currentData[EdkDll.EE_DataChannel_t.F7][i];
                        input[2] = currentData[EdkDll.EE_DataChannel_t.F3][i];
                        input[3] = currentData[EdkDll.EE_DataChannel_t.FC5][i];
                        input[4] = currentData[EdkDll.EE_DataChannel_t.T7][i];
                        input[5] = currentData[EdkDll.EE_DataChannel_t.P7][i];
                        input[6] = currentData[EdkDll.EE_DataChannel_t.O1][i];
                        input[7] = currentData[EdkDll.EE_DataChannel_t.O2][i];
                        input[8] = currentData[EdkDll.EE_DataChannel_t.P8][i];
                        input[9] = currentData[EdkDll.EE_DataChannel_t.T8][i];
                        input[10] = currentData[EdkDll.EE_DataChannel_t.FC6][i];
                        input[11] = currentData[EdkDll.EE_DataChannel_t.F4][i];
                        input[12] = currentData[EdkDll.EE_DataChannel_t.F8][i];
                        input[13] = currentData[EdkDll.EE_DataChannel_t.AF4][i];

                        this.rawData.Add(input);
                        //double time = currentData[EdkDll.EE_DataChannel_t.TIMESTAMP][i];
                        //Console.WriteLine(" AF4:" + AF4 + " F8:" + F8);
                    }
